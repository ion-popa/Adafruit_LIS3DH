...
#define LIS3DH_HIGH_RES_MODE_DATA_SCALE 16000


#define LIS3DH_WHO_AM_I_VALUE 0x33

#define LIS3DH_FIFO_MAX_DEPTH 31
...
typedef enum
{
    LIS3DH_INT_CFG_DISABLE = 0b000000,
    LIS3DH_INT_CFG_XLIE = 0b000001,
    LIS3DH_INT_CFG_XHIE = 0b000010,
    LIS3DH_INT_CFG_YLIE = 0b000100,
    LIS3DH_INT_CFG_YHIE = 0b001000,
    LIS3DH_INT_CFG_ZLIE = 0b010000,
    LIS3DH_INT_CFG_ZHIE = 0b100000,
}lis3dh_interruptEnable_t;

typedef enum
{
    LIS3DH_INT_SRC_XL = 0b0000001,
    LIS3DH_INT_SRC_XH = 0b0000010,
    LIS3DH_INT_SRC_YL = 0b0000100,
    LIS3DH_INT_SRC_YH = 0b0001000,
    LIS3DH_INT_SRC_ZL = 0b0010000,
    LIS3DH_INT_SRC_ZH = 0b0100000,
    LIS3DH_INT_SRC_IA = 0b1000000,
}lis3dh_interruptSource_t;


typedef enum
{
    LIS3DH_INT_MODE_OR = 0b0,
    LIS3DH_INT_MODE_MOVE = 0b01000000,
    LIS3DH_INT_MODE_AND = 0b10000000,
    LIS3DH_INT_MODE_POSITION = 0b11000000
}lis3dh_interruptMode_t;

typedef enum
{
    LIS3DH_PM_POWER_DOWN,
    LIS3DH_PM_LOW_POWER,
    LIS3DH_PM_NORMAL,
    LIS3DH_PM_HIGH_RES,
}lis3dh_powerMode_t;
...
typedef enum
{
    LIS3DH_ACTIVE_AXIS_X = 0b001,
    LIS3DH_ACTIVE_AXIS_XY = 0b011,
    LIS3DH_ACTIVE_AXIS_XYZ = 0b111,
    LIS3DH_ACTIVE_AXIS_XZ = 0b101,
    LIS3DH_ACTIVE_AXIS_Y = 0b010,
    LIS3DH_ACTIVE_AXIS_YZ = 0b110,
    LIS3DH_ACTIVE_AXIS_Z = 0b100,
}lis3dh_activeAxis_t;
...
typedef enum
{
    LIS3DH_FIFO_MODE_BYPASS = 0b00,
    LIS3DH_FIFO_MODE = 0b01,
    LIS3DH_FIFO_MODE_STREAM = 0b10,
...
public:
    ...
    bool begin(uint8_t addr = LIS3DH_DEFAULT_ADDRESS, lis3dh_dataRate_t dataRate = LIS3DH_DATARATE_50_HZ, lis3dh_powerMode_t powerMode = LIS3DH_PM_NORMAL,
        lis3dh_activeAxis_t activeAxis = LIS3DH_ACTIVE_AXIS_XYZ);

    uint8_t getDeviceID(void);
    bool haveNewData(void);
    bool enableDRDY(bool enable_drdy = true, uint8_t int_pin = 1);

    void read(uint8_t fifoIdx = 31); // fifoIdx=0 will read first (oldest) acc value. fifoIdx = 31 will read the last (newest) value.
    int16_t readADC(uint8_t a);

    void setRange(lis3dh_range_t range);
    lis3dh_range_t getRange(void);

    lis3dh_powerMode_t getPowerMode();
    void setPowerMode(lis3dh_powerMode_t powerMode);

    void enableAxes(lis3dh_activeAxis_t activeAxis);
    ...
    bool int1Cfg(lis3dh_interruptMode_t intMode, lis3dh_interruptEnable_t intEn, float threshold, float duration,
...
private:
    ...
    uint16_t _odr = LIS3DH_DATARATE_POWERDOWN;
    lis3dh_powerMode_t _powerMode = LIS3DH_PM_POWER_DOWN;
    uint32_t _dataScale = 1;
    lis3dh_range_t _range = LIS3DH_RANGE_2_G;

    float getOdrTime();
    uint8_t getDataResolution();
